...
public class ContentManagerTests
{
  ...
  private readonly FieldType _emailAddressType = new(new UniqueName(FieldType.UniqueNameSettings, "EmailAddress"), new StringSettings());
  private readonly FieldType _personNameType = new(new UniqueName(FieldType.UniqueNameSettings, "PersonName"), new StringSettings(minimumLength: 2, maximumLength: 100));
  ...
  public async Task Given_UniqueValueConflicts_When_SaveAsync_Then_ContentFieldValueConflictException()
  {
    ...
    _fieldValueValidatorFactory.Setup(x => x.Create(_emailAddressType)).Returns(new StringValueValidator((StringSettings)_emailAddressType.Settings));
  ...
  public async Task Given_UniqueNameConflict_When_SaveAsync_Then_ContentUniqueNameAlreadyUsedException()
  {
    ...
    Assert.Equal(content.ContentTypeId.ToGuid(), exception.ContentTypeId);
    Assert.Null(exception.LanguageId);
    Assert.Equal(conflict.Id.ToGuid(), exception.ConflictId);
    Assert.Equal(content.Id.ToGuid(), exception.EntityId);
