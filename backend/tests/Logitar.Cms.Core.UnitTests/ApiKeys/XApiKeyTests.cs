...
public class XApiKeyTests
{
  ...
  public void ctor_it_should_construct_the_correct_X_Api_Key_token()
  {
  ...
  [Fact(DisplayName = "Decode: it should decode the correct X-API-Key token.")]
  public void Decode_it_should_decode_the_correct_X_Api_Key_token()
  {
    ApiKeyId id = ApiKeyId.NewId();
    string secret = RandomStringGenerator.GetBase64String(XApiKey.SecretLength, out _);
    string value = $"AK.{id.Value}.{secret.ToUriSafeBase64()}";
  ...
  public void Encode_it_should_encode_correctly_the_X_Api_Key_token()
  {
    ApiKeyId id = ApiKeyId.NewId();
    string secret = RandomStringGenerator.GetBase64String(XApiKey.SecretLength, out _);
    XApiKey xApiKey = new(id, secret);

    string encoded = $"AK.{id.Value}.{secret.ToUriSafeBase64()}";
    Assert.Equal(encoded, xApiKey.Encode());
  }

  [Fact(DisplayName = "Encode: it should encode correctly the X-API-Key from parts.")]
  public void Encode_it_should_encode_correctly_the_X_Api_Key_token_from_parts()
  {
    ApiKeyId id = ApiKeyId.NewId();
    string secret = RandomStringGenerator.GetBase64String(XApiKey.SecretLength, out _);
