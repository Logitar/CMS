...
{
  public UpdateFieldTypeValidator(IUniqueNameSettings uniqueNameSettings)
  {
    ...
    RuleFor(x => x).Must(x => GetDataTypes(x).Count <= 1)
      .WithErrorCode("UpdateFieldTypeValidator")
  ...
  private static IReadOnlyCollection<DataType> GetDataTypes(UpdateFieldTypePayload payload)
  {
