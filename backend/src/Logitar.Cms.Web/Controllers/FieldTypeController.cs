...
public class FieldTypeController : ControllerBase
{
  ...
  public async Task<ActionResult<FieldType>> CreateAsync([FromBody] CreateFieldTypePayload payload, CancellationToken cancellationToken)
  {
    ...
    Uri location = HttpContext.BuildLocation("api/fields/types/{id}", new Dictionary<string, string>
    {
  ...
  [HttpPatch("{id}")]
  public async Task<ActionResult<FieldType>> UpdateAsync(Guid id, [FromBody] UpdateFieldTypePayload payload, CancellationToken cancellationToken = default)
  {
