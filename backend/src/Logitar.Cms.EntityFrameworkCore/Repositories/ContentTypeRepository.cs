...
internal class ContentTypeRepository : EventSourcing.EntityFrameworkCore.Relational.AggregateRepository, IContentTypeRepository
{
  ...
  public ContentTypeRepository(ISqlHelper sqlHelper, IEventBus eventBus, EventContext eventContext, IEventSerializer eventSerializer)
  ...
  public async Task<ContentTypeAggregate?> LoadAsync(ContentTypeId id, long? version, CancellationToken cancellationToken)
  {
    return await LoadAsync<ContentTypeAggregate>(id.AggregateId, version, cancellationToken);
  }
